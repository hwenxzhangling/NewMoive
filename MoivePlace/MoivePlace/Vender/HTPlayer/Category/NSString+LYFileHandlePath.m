...
+ (void)cleanVideoCache
{
    ...
    NSString *cachePath = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES).firstObject;
    NSString *cacheFolderPath = [cachePath stringByAppendingPathComponent:[NSString stringWithFormat:@"Moment_Videos"]];
    if ([manger fileExistsAtPath:cacheFolderPath])
    {
        if ([manger removeItemAtPath:cacheFolderPath error:nil])
        {
            [manger createDirectoryAtPath:cacheFolderPath withIntermediateDirectories:YES attributes:nil error:nil];
        }
    }
}
@end
