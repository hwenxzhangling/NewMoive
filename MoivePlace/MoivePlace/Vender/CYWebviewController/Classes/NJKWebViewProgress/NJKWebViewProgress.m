...
static NJKWebViewProgress *_webViewProgress = nil;
+(NJKWebViewProgress *)shaareNJKWebViewProgress;
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        {
            _webViewProgress = [[self alloc] init];
        }
    });
    
    _webViewProgress->_maxLoadCount = 0;
    _webViewProgress->_loadingCount = 0;
    _webViewProgress->_interactive = NO;
...
{
    //[self loadViewAppJSWebview:webView];//启动本地JSSDK
    ...
    if (navigationType == UIWebViewNavigationTypeLinkClicked)
    {
        ...
        if(webView.tag == KBDSearchWebViewTag)
        {
            if(self.progressDelegate && [self.progressDelegate respondsToSelector:@selector(pushURL:)])
            {
                [self.progressDelegate pushURL:url];
            }
            return NO;
        }
    }
